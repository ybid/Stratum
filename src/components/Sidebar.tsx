...
export function Sidebar() {
  ...
  return (
    <aside className="w-60 shrink-0 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 flex flex-col h-full select-none">
      {/* Header */}
      <div className="flex items-center justify-between px-3 h-10 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <button
          onClick={() => addDirectory(t(locale, 'directory'))}
          ...
          onAddTask={
            contextMenu.type === 'directory'
              ? () => {
                  ...
                  toggleDirectoryCollapse(contextMenu.id); // expand
                  setContextMenu(null);
                }
              : undefined
          }
